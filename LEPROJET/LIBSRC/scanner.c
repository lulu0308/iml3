...
Modified by Leon Mugwwaneza for the Logiciel de Base assignment.
The code which deals with the symbol table is removed.
...
static inst_info * map_string_to_inst_info (inst_info tbl[], int tbl_len, char *id);
...
static inst_info * map_string_to_inst_info(); 
static void * xmalloc ();
char * str_copy ();
#endif
...
#line 126 "scanner.l"
{
/*			strncpy(La_ligne.comment, yytext, MAX_LINE_SIZE-1);
*/
			scanner_store_comment(yytext, MAX_LINE_SIZE-1);
				/* Stores comments */
			}
	YY_BREAK
case 7:
...
#line 145 "scanner.l"
{
			  ...
			  if (*yytext == '-')
			    {
			      sscanf(yytext+3, "%lx", (unsigned long *)(&(yylval.i)));
			      yylval.i = -yylval.i;
			    }
			  else
			    {
			      sscanf(yytext+2, "%lx", (unsigned long *)(&(yylval.i)));
			    }
			  return (Y_INT);
			}
	YY_BREAK
...
/* 
Code to check for named constants removed
*/
...
#endif
{
  if (line_returned)
  ...
  else
    {
      ...
      /* Ran into null byte, probably from yylex.  Look further for newline */
      if (*eol1 == '\0')
	{
	  null1 = eol1;
	  *eol1 = ' ';
...
#ifdef __STDC__
int
map_inst_internal_code_to_internal_type (int code)
#else
int
map_inst_internal_code_to_internal_type (code)
int code ;
#endif
{

	int i;
	for (i=0; i< sizeof(keyword_tbl)/sizeof(inst_info); i++)
	{
		if (code == keyword_tbl[i].value1)
		{
			return(keyword_tbl[i].value2);
		}
	}
	return(OTHER_INST);
}
...
#ifdef __STDC__
void
map_inst_internal_code_to_name (int code, char *name)
#else
void
map_inst_internal_code_to_name (code, name)
int code,
char *name ;
#endif
{
        ...
        for (i=0; i< sizeof(keyword_tbl)/sizeof(inst_info); i++)
        {
                if (code == keyword_tbl[i].value1)
                {
                        strcpy(name, keyword_tbl[i].name);
			return ;
                }
        }
        strcpy(name, "");
